def main():
    ...
    cave = generate_cave_bottomless(input_list)
    print(simulate_sand(cave))
    cave = generate_cave_with_floor(input_list)
    print(simulate_sand(cave))
...
def simulate_sand(cave):
    ...
    num_sand = 0
    sand = start_point
    while sand[0] < len(cave) - 1 and cave[start_point[0]][start_point[1]] != "o":
        sand = start_point
...
def generate_cave_bottomless(input_list):
    max_length = 0
    max_depth = 0
...
def generate_cave_with_floor(input_list):
    ADDITIONAL_LENGTH = 500
    max_length = 0
    max_depth = 0
    for line in input_list:
        if line != "\n":
            ...
            for pair in line:
                ...
                if int(y) > max_depth:
                    max_depth = int(y)
    cave = [["." for _ in range(max_length + ADDITIONAL_LENGTH)]
